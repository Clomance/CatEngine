...
impl Window{
    ...
    #[inline(always)]
    pub fn destroy(window:WindowHandle)->bool{
        unsafe{
    ...
    #[inline(always)]
    pub fn get_device_context(window:Option<WindowHandle>)->Option<DeviceContextHandle>{
        unsafe{
    ...
    #[inline(always)]
    pub fn get_device_context_unchecked(window:Option<WindowHandle>)->DeviceContextHandle{
        unsafe{
    ...
    #[inline(always)]
    pub fn get_window_rectangle(window:WindowHandle,rectangle:&mut [i32;4])->bool{
        unsafe{
    ...
    /// In other words, the pixel at (right, bottom) lies immediately outside the rectangle.
    #[inline(always)]
    pub fn get_client_rectangle(window:WindowHandle,rectangle:&mut [i32;4])->bool{
        unsafe{
    ...
    #[inline(always)]
    pub fn set_window_position(window:WindowHandle,insert_after:Option<WindowHandle>,[x,y,width,height]:[i32;4],flag:u32)->bool{
        unsafe{
    ...
    #[inline(always)]
    pub unsafe fn set_window_long_ptr(window:WindowHandle,index:WindowData,value:isize)->isize{
    ...
    #[inline(always)]
    pub unsafe fn get_window_long_ptr(window:WindowHandle,index:WindowData)->isize{
    ...
    #[inline(always)]
    pub unsafe fn show_window(window:WindowHandle,command:ShowCommand)->bool{
    ...
    #[inline(always)]
    pub fn client_to_screen(window:WindowHandle,point:&mut [i32;2])->bool{
        unsafe{
    ...
    /// If the function fails, return `false`.
    #[inline(always)]
    pub fn screen_to_client(window:WindowHandle,point:&mut [i32;2])->bool{
        unsafe{
...
impl Window{
    ...
    /// To get extended error information, call `WinCore::get_last_error`.
    #[inline(always)]
    pub unsafe fn set_class_long_ptr(window:WindowHandle,index:ClassData,value:isize)->usize{
    ...
    /// To get extended error information, call `WinCore::get_last_error`.
    #[inline(always)]
    pub unsafe fn get_class_long_ptr(window:WindowHandle,index:ClassData)->usize{
        GetClassLongPtrW(window.as_raw(),index as i32)
    }
}

impl Window{
    /// Pepares the specified window for painting
    /// and fills a PAINTSTRUCT structure with information about the painting.
    /// 
    /// `window` - Handle to the window to be repainted.
    /// 
    /// `paint` - Pointer to the `PaintStruct` structure that will receive painting information.
    /// 
    /// The `begin_paint` function automatically sets the clipping region of the device context to exclude any area outside the update region.
    /// The update region is set by the InvalidateRect or InvalidateRgn function and by the system after sizing,
    /// moving, creating, scrolling, or any other operation that affects the client area.
    /// If the update region is marked for erasing, `begin_paint` sends a WM_ERASEBKGND message to the window.
    /// 
    /// An application should not call `begin_paint` except in response to a WM_PAINT message.
    /// Each call to `begin_paint` must have a corresponding call to the `end_paint` function.
    /// 
    /// If the window's class has a background brush, `begin_paint` uses that brush to erase the background of the update region before returning.
    /// 
    /// This API does not participate in DPI virtualization. The output returned is always in terms of physical pixels.
    /// 
    /// If the caret is in the area to be painted, `begin_paint` automatically hides the caret to prevent it from being erased.
    /// 
    /// If the function succeeds, the return value is the handle to a display device context for the specified window.
    /// 
    /// If the function fails, the return value is `None`, indicating that no display device context is available.
    #[inline(always)]
    pub unsafe fn begin_paint(window:WindowHandle,paint:&mut PaintStruct)->Option<DeviceContextHandle>{
        DeviceContextHandle::from_raw(BeginPaint(window.as_raw(),transmute(paint)))
    }

    /// Marks the end of painting in the specified window.
    /// This function is required for each call to the `begin_paint` function, but only after painting is complete.
    /// 
    /// `window` - Handle to the window that has been repainted.
    /// 
    /// `paint` - Pointer to a `PaintStruct` structure that contains the painting information retrieved by `begin_paint`.
    /// 
    /// If the caret was hidden by `begin_paint`, `end_paint` restores the caret to the screen.
    /// 
    /// `end_paint` releases the display device context that `begin_paint` retrieved.
    /// 
    /// The return value is always nonzero. So it's ignored.
    #[inline(always)]
    pub unsafe fn end_paint(window:WindowHandle,paint:&PaintStruct){
        // The return value is always nonzero.
        EndPaint(window.as_raw(),transmute(paint));
    }
}