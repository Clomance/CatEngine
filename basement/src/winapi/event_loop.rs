...
impl EventLoopManager{
    pub fn new(update_interval:i64,redraw_request_interval:i64)->EventLoopManager{
        Self{
            update_interval,
            redraw_request_interval,

            control:LoopControl::Run,
        }
    }
}
...
impl EventLoop{
    ...
    pub fn run<F:FnMut(Event,&mut EventLoopManager)>(&self,mut f:F){
        let mut manager=EventLoopManager::new(
            self.update_interval,
            self.redraw_request_interval
        );
        ...
        unsafe{
            ...
            // Начальное событие
            f(Event::Process(ProcessEvent::EventLoopStart),&mut manager);

            loop{
                match manager.control{
                    LoopControl::Run=>{
                        if PeekMessageW(&mut message,null_mut(),0,0,PM_REMOVE)==1{
                            match message.message{
                                WM_QUIT=>{
                                    f(Event::Process(ProcessEvent::Quit),&mut manager);
                                    break
                                }

                                _=>event_handler(&message,&mut f,&mut manager),
                            }
                        }
            ...
            // Завершение цикла
            f(Event::Process(ProcessEvent::EventLoopBreak),&mut manager);
        }
    }
}
...
fn event_handler<F:FnMut(Event,&mut EventLoopManager)>(message:&MSG,f:&mut F,manager:&mut EventLoopManager){
    unsafe{
