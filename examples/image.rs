...
impl WindowPage<'static> for Page{
    ...
    fn on_redraw_requested(&mut self,window:&Window){
        window.draw(&self.graphics,|graphics|{
            graphics.clear_colour([1.0;4]);
            // Drawing static image
            self.image_base.draw(&self.texture,graphics).unwrap();
            // Drawing rotating image
            self.image_base.draw_rotate(&self.texture,[200f32,200f32],self.angle,graphics).unwrap();
        }).unwrap();
    }
