...
use super::{SimpleGraphics, TextureGraphics, TextGraphics};
...
impl Objects{
    ...
    pub fn manager(&mut self,index:usize)->SystemObjectManager{
        SystemObjectManager{
            storage:self.get_storage(index)
        }
    }

    pub fn push(&mut self,system_storage_index_ptr:&mut usize){
    ...
    pub fn push_new(&mut self,system_storage_index_ptr:&mut usize,storage:SystemObjectStorage){
        *system_storage_index_ptr=self.systems.len();
...
pub struct SystemObjectManager<'a>{
    storage:&'a mut SystemObjectStorage
}

impl<'a> SystemObjectManager<'a>{
    pub (crate) fn new(storage:&'a mut SystemObjectStorage)->SystemObjectManager<'a>{
        SystemObjectManager{
            storage
        }
    }
    ...
    pub fn push_simple_object<O:SimpleObject>(
        ...
        match graphics.push_object(attributes,layer){
            Ok(render_object_id)=>Ok({
                let graphics=graphics as *mut SimpleGraphics as *mut ();
                let render_data=RenderDataInterface::new(graphics,layer,render_object_id);
    ...
    pub fn push_texture_object<O:TextureObject>(
        ...
        match graphics.push_object(attributes,layer){
            Ok(render_object_id)=>Ok({
                let graphics=graphics as *mut TextureGraphics as *mut ();
                let render_data=RenderDataInterface::new(graphics,layer,render_object_id);
    ...
    pub fn push_text_object<O:TextObject>(
        ...
        match graphics.push_object(attributes,layer){
            Ok(render_object_id)=>Ok({
                let graphics=graphics as *mut TextGraphics as *mut ();
                let render_data=RenderDataInterface::new(graphics,layer,render_object_id);
    ...
    pub fn push_simple_object_array<O:SimpleObject>(&mut self)->ArrayReference<'a,O>{
        let index=self.storage.len();

        let unit=ObjectUnit::array(8);
        self.storage.push(unit);

        ArrayReference::new(index,Vec::with_capacity(8))
    }

    pub fn push_texture_object_array<O:TextureObject>(&mut self)->ArrayReference<'a,O>{
        let index=self.storage.len();
    ...
    pub fn push_text_object_array<O:TextObject>(&mut self)->ArrayReference<'a,O>{
        let index=self.storage.len();
...
impl<'a,O> ArrayReference<'a,O>{
    pub (crate) fn new(index:usize,objects:Vec<ObjectReference<'a,O>>)->ArrayReference<'a,O>{
        Self{
            index,
            objects
        }
    ...
    pub fn remove(&mut self,index:usize,object_manager:&mut SystemObjectManager){
        let mut object_table=object_manager.storage.get_unit(self.index).remove(index);
        object_table.drop();
...
impl<'a,O:SimpleObject> ArrayReference<'a,O>{
    pub fn push_simple(
        &mut self,
        object:O,
        vertices:Vertices<SimpleVertex>,
        indices:Indices<ElementIndexType>,
        layer:usize,
        object_manager:&mut SystemObjectManager,
        graphics:&mut SimpleGraphics
    ){
        let attributes=ObjectAttributes::new(
            vertices.vertices,
        ...
        let render_object_id=graphics.push_object(attributes,layer).unwrap();
        ...
        object_manager.storage.get_unit(self.index).push(object_table);

        self.objects.push(ObjectReference::new(object_data));
    }
}

impl<'a,O:TextureObject> ArrayReference<'a,O>{
...
impl<'a,O:TextObject> ArrayReference<'a,O>{
    pub fn push_text(
        &mut self,
        object:O,
        vertices:Vertices<TextVertex>,
